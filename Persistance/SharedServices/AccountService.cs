    ...
    public class AccountService : IAccountService
    {
       ...
       // private readonly IEmailService _emailService;
        ...
        public async Task<ApiResponse<AuthenticationResponse>> Authenticate(AuthenticationRequest request)
        {
            ...
            if (user == null)
            {
                throw new ApiExceptions($"User not registered with this {request.Email}");
            }

            if (!user.EmailConfirmed)
            {
                throw new ApiExceptions($"Email not confirmed, pls confirm your email to login");
            }

            if (await _userManager.IsLockedOutAsync(user))
            {
                throw new ApiExceptions($"Your account is locked, please try agina later.");
            }
        ...
        public async Task<ApiResponse<Guid>> RegisterUser(RegisterRequest registerRequest)
        {
            ...
            if (user != null)
            {
                throw new ApiExceptions($"User already taken {registerRequest.Email}");
            }
...
";
        ...
        public async Task<ApiResponse<bool>> ConfirmEmail(string userId, string token)
        {
            ...
            if (user == null)
            {
                throw new ApiExceptions($"User not found with this {userId}");
            }
        ...
        public async Task<ApiResponse<bool>> ResendConfirmationEmailAsync(string email)
        {
            ...
            if (user == null)
            {
                throw new ApiExceptions($"User not found with this {email}");
            }

            if (user.EmailConfirmed)
            {
                throw new ApiExceptions($"Email already confirmed");
            }
        ...
        public async Task<ApiResponse<bool>> ForgotPasswordAsync(string userEmail)
        {
            ...
            if (user == null)
            {
                throw new ApiExceptions($"User not found with this {userEmail}");
            }
        ...
        public async Task<ApiResponse<bool>> ResetPasswordAsync(ResetPasswordRequest resetPassword)
        {
            ...
            if (user == null)
            {
                throw new ApiExceptions($"User not found with this {resetPassword.Email}");
            }
